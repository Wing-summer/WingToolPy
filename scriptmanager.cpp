...
ScriptManager::ScriptManager(QObject *parent) : QObject(parent) {
  auto dir = PLUGINDIR + "/PyScript";
  QDir pdir(dir);
  if (!pdir.exists()) {
    return;
  }
  pdir.setFilter(QDir::Filter::Dirs | QDir::Filter::NoDotAndDotDot);
  auto fdirs = pdir.entryInfoList();
  for (auto folder : fdirs) {
    auto na = folder.fileName();
    QDir jdir(folder.filePath());
    QList<ScriptMeta> m;
    jdir.setFilter(QDir::Filter::Dirs | QDir::Filter::NoDotAndDotDot);
...
void ScriptManager::loadMenu(QMenu *menu) {
  ...
  for (; k != ke; k++) {
    ...
    for (auto mitem : n.second) {
      QAction *a = new QAction(mitem.name, this);
      connect(a, &QAction::triggered, this,
              [=] { emit this->runPyFile(mitem.filename); });
      i->addAction(a);
    }
    menu->addMenu(i);
  }
...
void ScriptManager::loadTreeWidget(QTreeWidget *tree) {
  ...
  for (; k != ke; k++) {
    ...
    p->setIcon(0, QIcon(":/WingToolPy/img/pydb.png"));
    for (auto mitem : n.second) {
      auto i = new QTreeWidgetItem(p, {mitem.name});
      i->setIcon(0, QIcon(":/WingToolPy/img/py.png"));
